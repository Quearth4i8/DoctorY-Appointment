...
export async function GET(req: Request) {
  ...
  return handleStaff((doctorId) => {
    if (!from || !to) {
      throw new FrontDeskError(400, "Plage de dates manquante.");
    }
    return listAppointments(doctorId, from, to);
  });
}
...
export async function POST(req: Request) {
  ...
  return handleStaff((doctorId) => {
    if (!body.patient_id || !body.appointment_datetime) {
      throw new FrontDeskError(400, "Patient et date/heure obligatoires.");
    }
