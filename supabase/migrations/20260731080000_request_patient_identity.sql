-- The public form now asks whether the visitor is already a patient, and if so
-- for their numéro de dossier — which is a far better key for matching than a
-- phone number shared across a family.
--
-- The motif box is gone (the secretary asks on the phone) and so are the day /
-- period pickers: a request now always carries the exact slot the visitor
-- clicked on the availability calendar.

alter table public.appointment_requests
  add column if not exists is_existing_patient boolean not null default false,
  add column if not exists numero_dossier text not null default '';

create index if not exists appointment_requests_dossier_idx
  on public.appointment_requests (numero_dossier)
  where numero_dossier <> '';

-- Recreate the intake function with the two new arguments. Postgres overloads
-- on signature, so the previous one has to go or both would exist.
...
begin
  ...
  -- Claiming to be an existing patient without a file number gives the
  -- secretary nothing to match on.
  ...
  insert into appointment_requests (
    ...
    is_existing_patient, numero_dossier
  ) values (
    btrim(p_last_name),
